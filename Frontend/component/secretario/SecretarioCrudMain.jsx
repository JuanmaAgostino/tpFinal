...
import '../../styles/SecretarioCrud.css';
...
export default function SecretarioCrudMain() {
	...
	const handleEliminar = async (idAlumno, idUsuario) => {
		if (!window.confirm('¿Seguro que deseas eliminar este alumno y su usuario?'))
			return;
		try {
	...
	// Filtrado por legajo
	const alumnosFiltrados = alumnosConUsuarios.filter(
		(a) =>
			!busquedaLegajo || (a.Legajo && a.Legajo.toString().includes(busquedaLegajo))
	);
	...
	return (
		<div className="secretaria-container">
			...
			<form onSubmit={handleCrearUsuario} className="secretaria-form">
				...
				<input
					...
					disabled={usuarioCreado !== null}
				/>
				<button type="submit" disabled={usuarioCreado !== null}>
					Crear Usuario
				</button>
			</form>
			...
			{usuarioCreado && (
				<form onSubmit={handleCrearAlumno} className="secretaria-form">
					...
					<input
						...
						onChange={(e) =>
							setNuevoAlumno({ ...nuevoAlumno, Legajo: e.target.value })
						}
					/>
			...
			{/* Buscador por legajo */}
			<div className="secretaria-search">
				<input
					type="text"
			...
			<div className="secretaria-list">
				<ul>
					{alumnosFiltrados.map((a) => (
						<li key={a.idAlumno}>
							<div className="info">
								{a.nombre} {a.apellido} - Legajo: {a.Legajo} - Usuario: {a.Usuario} -
								Email: {a.Email}
							</div>
							<div>
								<button onClick={() => handleEditar(a)}>Editar</button>
								<button onClick={() => handleEliminar(a.idAlumno, a.idUsuario)}>
									Eliminar
								</button>
							</div>
						</li>
			...
			{editando && (
				<form onSubmit={handleGuardar} className="secretaria-form">
					...
					<input
						...
						onChange={(e) =>
							setFormUsuario({ ...formUsuario, Contraseña: e.target.value })
						}
					/>
					<input
						name="Email"
					...
					<input
						...
						onChange={(e) =>
							setFormAlumno({ ...formAlumno, nombre: e.target.value })
						}
					/>
					<input
						name="apellido"
						...
						onChange={(e) =>
							setFormAlumno({ ...formAlumno, apellido: e.target.value })
						}
					/>
					<input
						name="Legajo"
						...
						onChange={(e) =>
							setFormAlumno({ ...formAlumno, Legajo: e.target.value })
						}
					/>
